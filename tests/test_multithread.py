...
from cdstemplate.multithread import tokenize, process_file, main
...
def test_parallel_processing_different_workers(tmp_path):
    ...
    for i in range(10):  # Create 10 files
        (test_dir / f"file{i}.txt").write_text(f"a b c d e f g h i j k l m n o p q r s t u v w x y z")
    ...
    assert processing_times[0] >= processing_times[-1] * 0.5  # Single worker should take at least half the time of max workers
